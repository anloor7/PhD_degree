...
# Removing the series whose length is less than 1258 and the corresponding labels 
...
coherence2 <- listTomatrix(lapply(mts, quantile_quantities_re_im))
dis_matrix <- proxy::dist(coherence2, EuclideanDistance) 
pamc <- pam(dis_matrix, 2)
clustering <- pamc$cluster
external_validation(ground_truth_2_sectors, clustering, summary_stats = T)
cluster.evaluation(ground_truth_2_sectors, clustering)
loo1nn.cv(dis_matrix, ground_truth_2_sectors)
table(clustering, ground_truth_2_sectors) # Contingency table

pamc$medoids # Medoids
pamc$id.med # Indexes of medoids 
index_2_sectors[pamc$id.med]
where_2_sectors_symbol[pamc$id.med] # Medoids, D, utilities, USB, finantials
...
J <- 7
w <- 'd4'
features <- lapply(mts, wavelet_features, wf = w, J = J) 
...
dis_matrix <- matrix(0, 80, 80)
clustercov <- foreach (m = 1 : length(mts)) %dopar% {
  cov(mts[[m]])
}
...
for (i in 1:9) {
  ...
  plot_financials_9[[i]] <- mts_plot(mts_financials_9[[i]], title = financials_9[i]) + theme(axis.text = 
                                                                                               element_text(size = x_size),
                                                                                             plot.title = element_text(hjust = 0.5, size = title_size))
  plot_utilities_9[[i]] <- mts_plot(mts_utilities_9[[i]], title = utilities_9[i]) + theme(axis.text = 
                                                                                            element_text(size = x_size),
                                                                                          plot.title = element_text(hjust = 0.5, size = title_size))
...
# Graphical representation of the medoids 

mts_d <- mts[[27]]
mts_usb <- mts[[76]]

plot_d <- mts_plot(mts_d, title = 'D') + theme(axis.text = 
                                                 element_text(size = x_size),
                                               plot.title = element_text(hjust = 0.5, size = title_size))
plot_usb <- mts_plot(mts_usb, title = 'USB') + theme(axis.text = 
                                                       element_text(size = x_size),
                                                     plot.title = element_text(hjust = 0.5, size = title_size))
grid.arrange(plot_d, plot_usb)
