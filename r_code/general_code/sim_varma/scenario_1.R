...
set.seed(1234)
for (i in 1 : 50) {
  cluster1[[i]] <- VARMAsim(25, arlags = 1, phi = phi_c1, sigma = sigma_c1)$series
}

for (i in 1 : 50) {
  cluster2[[i]] <- VARMAsim(25, malags = 1, theta = theta_c2, sigma = sigma_c2)$series
}

for (i in 1 : 50) {
  cluster3[[i]] <- VARMAsim(25, arlags = 1, malags = 1, phi = phi_c3,
