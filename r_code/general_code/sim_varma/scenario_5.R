...
# Comparing QAF, QC1 and QC2. Scenario 5. 
...
set.seed(1234)
for (i in 1 : 50) {
  cluster1[[i]] <- VARMAsim(200, arlags = 1, malags = 1,phi = phi_c1, theta = theta_c1, sigma = sigma_c1)$series
}

for (i in 1 : 50) {
  cluster2[[i]] <- VARMAsim(200, arlags = 1, malags = 1, phi = phi_c2, theta = theta_c2, sigma = sigma_c2)$series
}

for (i in 1 : 50) {
  cluster3[[i]] <- VARMAsim(200, arlags = 1, malags = 1,phi = phi_c3, theta = theta_c3, sigma = sigma_c3)$series
}

for (i in 1 : 50) {
  cluster4[[i]] <- VARMAsim(200, arlags = 1, malags = 1,phi = phi_c4, theta = theta_c4, sigma = sigma_c4)$series
}
