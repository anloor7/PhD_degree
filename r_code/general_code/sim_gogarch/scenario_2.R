...
a1 <- garch.sim(alpha = c(0.01, 0.4), beta = 0.55, n = 8000)
b1 <- garch.sim(alpha = c(0.01, 0.4), beta = 0.55, n = 8000)
df1 <- cbind(a1, b1)
a2 <- garch.sim(alpha = c(0.01, 0.5), beta = 0.3, n = 8000)
b2 <- garch.sim(alpha = c(0.01, 0.5), beta = 0.3, n = 8000)
df2 <- cbind(a2, b2)
a3 <- garch.sim(alpha = c(0.01, 0.6), beta = 0.05, n = 8000)
b3 <- garch.sim(alpha = c(0.01, 0.6), beta = 0.05, n = 8000)
df3 <- cbind(a3, b3)
...
for (i in 1 : 100) {
  sim1 <- gogarchsim(fit1, n.sim = 2000)
  l1 <- slot(sim1, 'msim')
  cluster1[[i]] <- l1$seriesSim[[1]]
}

for (i in 1 : 100) {
  sim2 <- gogarchsim(fit2, n.sim = 2000)
  l2 <- slot(sim2, 'msim')
  cluster2[[i]] <- l2$seriesSim[[1]]
}

for (i in 1 : 100) {
  sim3 <- gogarchsim(fit3, n.sim = 2000)
  l3 <- slot(sim3, 'msim')
