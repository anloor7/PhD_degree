...
vectormvspec <- list(dcca1 = 0.1, dccb1 = 0.8) # Parameters of DCC model 
...
model <- multispec(replicate(4,uspec)) # Replication of eGarch model for the four variables 
vectormvspec <- list(dcca1 = 0.8, dccb1 = 0.1) # Parameters of DCC model 
mvspec <- dccspec(model, dccOrder = c(1, 1), model = "DCC", distribution = # Defining DCC model 
                    "mvnorm", fixed.pars = vectormvspec)
cluster2 <- list()
...
vectoruspec <- list(omega = 0.2, alpha1 = -0.8, beta1 = 0.8, gamma1 = 0.9) # Parameters of eGARCH model 
uspec <- ugarchspec(variance.model = list(model = "eGARCH", garchOrder = c(1, 1)),
                    distribution.model = "norm", mean.model = list(armaOrder=c(0, 0), include.mean = F), 
                    fixed.pars = vectoruspec)
...
vectormvspec <- list(dcca1 = 0.1, dccb1 = 0.1) # Parameters of DCC model 
mvspec <- dccspec(model, dccOrder = c(1, 1), model = "DCC", distribution = # Defining DCC model 
                    "mvnorm", fixed.pars = vectormvspec)
cluster4 <- list()
