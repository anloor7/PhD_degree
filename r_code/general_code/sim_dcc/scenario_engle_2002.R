...
# Lets perform simulations of scenarios based on Engle (2002), Section 5 

# 4 Clusters, 2 variables, 5 MTS per cluster.

# We can choose either the multivariate t distribution or the multivariate skewed normal 
...
for (i in 1 : l) {
  ...
  sigma3[[i]] <- diag(2)
  sigma3[[i]][1, 2] <- if (i < (l/2)) {0.9} else {0.4}
...
for (j in 1 : n){
  ...
  for (i in 1 : l) {
      ...
      # epsilon[[i]] <- rmvt(1, sigma = sigma4[[i]]) # Student
...
J <- 8 # number of scales (see Table 3, page 45, in D'urso and Maharaj 2012)
wf <- "d4"
