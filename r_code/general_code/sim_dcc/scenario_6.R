...
vectoruspec <- list(omega = 0.5, alpha1 = 0.1, beta1 = 0.1, ar1 = 0.2) # Parameters of eGARCH model 
uspec <- ugarchspec(variance.model = list(model = "fGARCH", submodel = 'GARCH', garchOrder = c(1, 1)),
                    distribution.model = "norm", mean.model = list(armaOrder=c(1, 0), include.mean = F), 
...
vectoruspec <- list(omega = 0.9, alpha1 = 0.38, beta1 = 0.55, ar1 =  0.2) # Parameters of eGARCH model 
uspec <- ugarchspec(variance.model = list(model = "fGARCH", submodel = 'GARCH', garchOrder = c(1, 1)),
                    distribution.model = "norm", mean.model = list(armaOrder=c(1, 0), include.mean = F), 
                    fixed.pars = vectoruspec)
...
# Cluster 3
vectoruspec <- list(omega = 0.4, alpha1 = 0.6, beta1 = 0.1, ar1 = 0.2) # Parameters of eGARCH model 
uspec <- ugarchspec(variance.model = list(model = "fGARCH", submodel = 'GARCH', garchOrder = c(1, 1)),
                    distribution.model = "norm", mean.model = list(armaOrder=c(1, 0), include.mean = F), 
                    fixed.pars = vectoruspec)
...
vectormvspec <- list(dcca1 = 0.9, dccb1 = 0.08) # Parameters of DCC model 
mvspec <- dccspec(model, dccOrder = c(1, 1), model = "DCC", distribution = # Defining DCC model 
                    "mvnorm", fixed.pars = vectormvspec)
cluster3 <- list()
